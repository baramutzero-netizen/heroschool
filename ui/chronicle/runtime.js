...
function chronHeading(title,cls=''){return `<div class="chron-heading ${cls}">${chronArt('leaf')}<h3>${title}</h3>${chronRule()}</div>`;}
...
function viewPlan(){
  ...
  const days=DAY_N.map((day,i)=>{const c=slots[i],tr=c&&(TR[c.t]||TR.free),col=c&&cardColor(c),mk=c&&markOf(c),chain=chainOf(slots,i);
    return `<div class="dslot chron-day ${c?'filled':''}" data-slot="${i}" style="--day-color:${col?col.c:'#9c8972'}"><b class="chron-weekday">${day}</b>${c?`<div class="chron-entry" draggable="true" data-card="${c.u}" title="클릭하면 손패로 돌아갑니다"><strong>${esc(tr.n)}</strong>${chronTrainingArt(c.t)}<small>${esc(tr.d)}</small>${mk?`<span class="chron-mark" style="background:${mk.c}" title="${esc(mk.d)}">${mk.i}</span>`:''}</div>`:'<span class="chron-empty">손패를 눌러 넣거나 이곳으로 끌어오세요</span>'}${chain?`<span class="chron-chain">체인 +${Math.round(chain*100)}%</span>`:''}${chronRule('chron-day-rule')}</div>`}).join('');
